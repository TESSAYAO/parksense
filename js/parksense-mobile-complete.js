...
async function autoLoadData() {
  ...
  try {
    ...
    document.getElementById('dataStatus').textContent = 'Loaded 6/6 layers';
  ...
  Object.keys(LAYER_FILES).forEach(key => {
    ...
    item.innerHTML = `
      ...
      <div class="info">
        ...
        <div class="status">Waiting to load</div>
      </div>
    `;
    ...
    item.onclick = function() {
      ...
      showToast(`Layer ${this.classList.contains('active') ? 'enabled' : 'disabled'}`, 'info');
    };
...
async function loadLayer(key, filename) {
  ...
  if (layerItem) {
    layerItem.querySelector('.status').textContent = 'Loading...';
  }
  ...
  try {
    ...
    if (key === 'boundary') {
      try {
        ...
        console.warn(`Failed to adjust view:`, e);
      }
    }
    ...
    if (layerItem) {
      ...
      layerItem.querySelector('.status').textContent = 'Loaded';
    }
    
  } catch (e) {
    console.error(`[${key.toUpperCase()}] Loading failed:`, e);
    if (layerItem) {
      layerItem.querySelector('.status').textContent = 'Load failed';
    }
  }
}
...
function buildTrailGraph(trailFeatures) {
  ...
  console.log(`Built trail graph: ${graph.nodes.size} nodes, ${graph.edges.length} edges`);
  return graph;
}
...
function setStartPoint(latlng) {
  ...
  pickMode = 'end';
  showPickHint('Please click on the map to select the end point');
  showToast('Start point set, please select end point', 'success');
}
...
function setEndPoint(latlng) {
  ...
  pickMode = null;
  hidePickHint();
  showToast('End point set, planning route...', 'info');
...
function planRoute() {
  if (!startLatLng || !endLatLng) {
    showToast('Please set start and end points first', 'warning');
    return;
  }
  ...
  if (trailGraph && trailGraph.nodes.size > 0) {
    ...
    if (startNode && endNode) {
      ...
      if (path && path.length > 1) {
        ...
        showToast('Route planning completed', 'success');
  ...
  showRouteInfo(distance, time);
  showToast('Generated straight-line route', 'warning');
...
function showRouteInfo(distance, time) {
  ...
  document.getElementById('routeTime').textContent = `${Math.round(time / 60)} min`;
    ...
    <div class="route-recommendations">
      <div class="route-option selected">
        <div class="route-option-header">
          <div class="route-option-name">🌸 Scenic Route</div>
          <div class="route-option-time">${Math.round(baseTime * 1.2 / 60)} min</div>
        </div>
        <div class="route-option-features">Passes major attractions and gardens, perfect for sightseeing</div>
      </div>
      
      <div class="route-option">
        <div class="route-option-header">
          <div class="route-option-name">⚡ Direct Route</div>
          <div class="route-option-time">${Math.round(baseTime * 0.8 / 60)} min</div>
        </div>
        <div class="route-option-features">Shortest path, quickly reach destination</div>
      </div>
      
      <div class="route-option">
        <div class="route-option-header">
          <div class="route-option-name">🏃 Fitness Route</div>
          <div class="route-option-time">${Math.round(baseTime * 1.5 / 60)} min</div>
        </div>
        <div class="route-option-features">Passes fitness facilities and exercise areas</div>
      </div>
    </div>
  `;
...
async function loadWeather() {
  try {
    ...
    const weatherText = `${weather.condition} ${weather.temperature}°C, Humidity ${weather.humidity}%, Wind ${weather.wind}km/h`;
    document.getElementById('weatherInfo').textContent = `Weather: ${weatherText}`;
    
  } catch (e) {
    document.getElementById('weatherInfo').textContent = 'Weather: Failed to load';
  }
}
...
async function reloadPOI() {
  ...
  try {
    ...
    // 更新POI计数显示
    document.getElementById('poiCount').textContent = `${finalFeatures.length} POI points`;
    
    showToast(`POI reload completed: ${finalFeatures.length} points`, 'success');
    
  } catch (e) {
    console.error('[POI] Reload failed:', e);
    showToast('POI reload failed', 'error');
  }
}
...
function setupPoiControls() {
  ...
  if (applyBtn) {
    applyBtn.onclick = function() {
      ...
      reloadPOI();
      showToast('POI filters applied', 'success');
    };
  }
...
function autoLoadAll() {
  ...
  showToast('All layers enabled', 'success');
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.startPickStart = function() {
    pickMode = 'start';
    closeAllDrawers();
    showPickHint('Please click on the map to select start point');
    showToast('Please select start point on the map', 'info');
  };
  
  window.startPickEnd = function() {
    pickMode = 'end';
    closeAllDrawers();
    showPickHint('Please click on the map to select end point');
    showToast('Please select end point on the map', 'info');
  };
